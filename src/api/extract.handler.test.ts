...
describe("POST /api/v1/extract", () => {
  ...
  it("returns extraction result for JSON body with URL", async () => {
    ...
    expect(frames.files).toHaveLength(3);
    expect(frames.urls).toHaveLength(3);
  ...
  it("uses LLM preset by default", async () => {
    const app = await buildApp({ env: createTestEnv(), logger: false });
    ...
    const extractCall = mockExtractFrames.mock.calls[0]?.[0];
    expect(extractCall?.preset).toEqual({
      width: 1024,
      format: "jpeg",
      quality: 80,
    });
  ...
  it("returns 400 for missing fps", async () => {
    const app = await buildApp({ env: createTestEnv(), logger: false });
  ...
  it("returns 400 for fps out of range", async () => {
    const app = await buildApp({ env: createTestEnv(), logger: false });
    ...
    const response = await app.inject({
      ...
      url: "/api/v1/extract",
      headers: { "content-type": "application/json" },
      payload: { source: "https://example.com/video.mp4", fps: 31 },
    });
  ...
  it("returns 413 when video exceeds max duration", async () => {
    mockGetVideoMetadata.mockResolvedValue({
      ...sampleMetadata,
      duration: 700, // exceeds 600s default
    });
  ...
  it("returns extraction result for multipart file upload", async () => {
    const app = await buildApp({ env: createTestEnv(), logger: false });
